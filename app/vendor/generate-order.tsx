...
export default function GenerateOrder() {
  ...
  const handleNumberPress = (num: string) => {
    ...
    if (amount.split('.')[1]?.length >= 2 && num !== 'backspace') return;
  ...
  const handleGenerateQR = async () => {
    ...
    try {
        ...
        const amountInWei = Math.floor(parseFloat(amount) * 1_000_000).toString();
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="px-6 pt-8">
        ...
        {/* Numeric Keypad */}
        <View className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-6 shadow">
            ...
            {['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0', '⌫'].map(
              (key) => (
                <TouchableOpacity
                  ...
                  className="w-1/3 aspect-square p-4 active:opacity-70"
                >
                  <View className="flex-1 items-center justify-center bg-gray-50 dark:bg-gray-700 rounded-xl">
          ...
          >
            {loading ? (
              ...
              <Text className="text-white text-xl font-bold text-center">
                {qrData ? 'QR Code Generated ✓' : 'Generate QR Code'}
              </Text>
            )}
          </TouchableOpacity>
